<?php
	...
	if (isAuthenticated()) {
		...
		$qry = "SELECT COUNT(*) AS myopenquestions " .
				...
				"AND A.id NOT IN (SELECT B.questionid FROM ols_questionanswers B WHERE B.questionid = A.id)";
		$result = mysql_query($qry);
		...
		$qry = "SELECT COUNT(*) AS myansweredquestions " .
				"FROM ols_question A " .
				"WHERE A.memberid = $memberid " .
				"AND A.id IN (SELECT B.questionid FROM ols_questionanswers B WHERE B.questionid = A.id)";
		$result = mysql_query($qry);
