...
?>
<form method="POST" class="entryform" id='articleform' action='askquestionpersist.php<?php if (isset($_GET['id'])) echo "?id=" . $_GET['id']; ?>'>
<?php 
	}
?>
	...
	<textarea required='true' id="questionbody" name="questionbody" rows="15" cols="60" style="height:340px;width: 340px" class="tinyMCE"></textarea>
...
<script type="text/javascript">
	function verify() {
		...
		if (tinyMCE.get('questionbody').getContent() == "") {
			$("#questionbody_tbl").addClass("invalid");
			isValid = false;
			
		} else {
			$("#questionbody_tbl").removeClass("invalid");
		}
	...
	$(document).ready(function() {
			...
			<?php 
				if (isset($_GET['id'])) {
					$id = $_GET['id'];
		    		$qry = "SELECT * FROM ols_question WHERE id = $id";
					$result = mysql_query($qry);
			...
			?>
							...
							$("#questionbody").val("<?php echo escape_notes($member['body']); ?>").trigger("change");
			<?php
