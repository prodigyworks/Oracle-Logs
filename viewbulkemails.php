	...
	<table width=100% class='grid list' id="xx" cellspacing=0 cellpadding=0>
	    ...
	    <tbody>
	    	<?php
	    		...
	    		$prevpage = ($fromrow > 0);
	    		$endrow = $torow + 1;
	    		$qry = "SELECT A.id, A.name, A.body, " .
	    				"DATE_FORMAT(A.createddate, '%d/%m/%Y') AS createddate " .
						"FROM ols_bulkemail A " . 
	    				"ORDER BY A.id DESC " .
				...
				if ($result) {
					while (($member = mysql_fetch_assoc($result))) {
						...
						echo "<td><a href='viewbulkemail.php?callee=" . base64_encode(basename($_SERVER['PHP_SELF'])) . "&id=" . $member['id'] . "'>" . $member['name'] . "</a></td>";
						echo "<td width='20px'><div id='prev' style='display:none'>" . $member['body'] . "</div><img  onclick='viewbody(this)' src='images/view.png'  title='View bulk email' /></td>";
						echo "<td width='20px'><a href='viewbulkemail.php?callee=" . base64_encode(basename($_SERVER['PHP_SELF'])) . "&id=" . $member['id'] . "'><img src='images/user.png'  title='View members' /></a></td>";
						echo "<td width='20px'><a href='resendbulkemail.php?callee=" . base64_encode(basename($_SERVER['PHP_SELF'])) . "&id=" . $member['id'] . "'><img src='images/recycle.png'  title='Resend' /></a></td>";
						echo "</tr>\n";
					}
				} else {
